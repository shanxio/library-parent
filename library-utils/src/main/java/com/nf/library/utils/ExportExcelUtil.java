...
public class ExportExcelUtil<T>{
     ...
     */
    public void exportExcel(String title, Class<?> pojo ,Collection<T> dataset, OutputStream out, String version) {
        if(StringUtils.isEmpty(version) || EXCEL_FILE_2003.equals(version.trim())){
            exportExcel2003(title,pojo,dataset, out, "yyyy-MM-dd HH:mm:ss");
        }else{
            exportExcel2007(title, pojo, dataset, out, "yyyy-MM-dd HH:mm:ss");
        }
    }
    ...
    public void exportExcel2007(String title, String[] headers, Collection<T> dataset, OutputStream out, String pattern) {
        ...
        String execl = "^//d+(//.//d+)?$";
        Pattern p = Pattern.compile(execl);
        Matcher matcher;
        String fieldName;
    ...
    public void exportExcel2003(String title,Class<?> clz, Collection<T> dataset, OutputStream out, String pattern) {
        // 声明一个工作薄
        HSSFWorkbook workbook = new HSSFWorkbook();
        // 生成一个表格
        HSSFSheet sheet = workbook.createSheet(title);
     ...
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void exportExcel2007(String title, Class<?> pojo, Collection<T> dataset, OutputStream out, String pattern) {
        // 声明一个工作薄
        XSSFWorkbook workbook = new XSSFWorkbook();
        // 生成一个表格
        XSSFSheet sheet = workbook.createSheet(title);
        ...
        XSSFRow row = sheet.createRow(0);
        XSSFCell cellHeader;
        Field[] clzFiles = pojo.getDeclaredFields();
        for (int i = 0;i<clzFiles.length;i++) {
            if(clzFiles[i].isAnnotationPresent(ExcelConfig.class)) {
                ExcelConfig excelConfig = clzFiles[i].getAnnotation(ExcelConfig.class);
                cellHeader = row.createCell(i);
